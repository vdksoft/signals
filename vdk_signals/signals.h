...
class signal<_ResT(_ArgTs...)> final
{
    ...
    class slot_functor final
    {
        ...
        {
            return (*reinterpret_cast<const target_slot<::std::nullptr_t, _Signature>*>(data)
                         ->mp_function)(args...);
        }
        ...
        {
            return (
                reinterpret_cast<const target_slot<_Class, _Signature>*>(data)->mp_instance
                    ->*reinterpret_cast<const target_slot<_Class, _Signature>*>(data)
                    ->mp_function)(args...);
        }
        ...
        {
            return (
                *reinterpret_cast<const target_slot<_Class, ::std::nullptr_t>*>(data)->mp_instance)(
                args...);
        }
    ...
    public:
            ...
            , mp_invoker{nullptr}
        {
            ...
            auto storage =
                reinterpret_cast<target_slot<::std::nullptr_t, _Signature>*>(&m_target[0]);
        ...
        explicit slot_functor(_Class* functor) noexcept
            ...
            , mp_invoker{nullptr}
        {
            auto storage = reinterpret_cast<target_slot<_Class, ::std::nullptr_t>*>(&m_target[0]);
        ...
        // Invoke target slot
        _ResT operator()(_ArgTs&... args) const { return (*mp_invoker)(&m_target[0], args...); }
    ...
    public:
        ...
        connection(const slot_functor& slot, const track_ptr& t_ptr, bool trackable) noexcept
            : m_slot(slot)
            , m_track_ptr(t_ptr)
            , mp_next(nullptr)
            , mp_deleted(nullptr)
    ...
    {
        return read_guard(
            m_sync_stage.load() == sync_stage::sync_stage_1 ? m_access_s1 : m_access_s2);
    }
    ...
    // May throw exception if memory allocation fails
    // Must be called under write_access() protection
    bool connect(const slot_functor& slot, const track_ptr& t_ptr, bool trackable)
    {
        synchronize();
...
class signal<_ResT(_ArgTs...)> final
{
    ...
    void expand_storage()
    {
        ...
        connection* new_block =
            reinterpret_cast<connection*>(VDK_NEW(new_capacity * sizeof(connection)));
